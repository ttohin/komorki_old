...
void PixelMapManager::CreateMap(cocos2d::Node* superView)
{
  ...
  m_bg->setPosition(size.x * spriteSize * 0.5 - spriteSize * 0.5,
                    size.y * spriteSize * 0.5 - spriteSize * 0.5);
  m_bg->setOpacity(120);
  ...
  for (int i = 0; i < stepsX; ++i)
  {
    for (int j = 0; j < stepsY; ++j)
    {
      ...
      m_maps.emplace_back(new PixelMapPartial(i*m_mapSegmentSize, j*m_mapSegmentSize, width, height, &m_provider));
  ...
  if (PMM_ENABLE_LOGGING)
  {
    for (auto u : result)
    {
      ...
      log("%s [%d:%d]->[%d:%d] [id:%d] t:%d",
          operationType.c_str(), (int)initialPos.x, (int)initialPos.y, (int)destinationPos.x, (int)destinationPos.y, descriptor->m_id, descriptor->GetType());
    }
  }
